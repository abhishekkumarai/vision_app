...
class CameraPlugin extends CameraPlatform {
  ...
  Future<List<CameraDescription>> availableCameras() async {
    try {
      ...
      // LOCAL PATCH (vision_app): upstream aborts the whole enumeration when a
      // single device fails to open (e.g. "OBS Virtual Camera" throws
      // NotReadableError whenever OBS isn't running), so no camera is usable.
      // Skip unopenable devices instead; rethrow only if none could be opened.
      ...
      if (cameras.isEmpty && firstDeviceError != null) {
